...
class rating_repo(db_class):
    ...
    class Rating(Base):
        ...
        header_cmt = Column(String(20), nullable=False)
    ...
    def give_rating(
        self, login_id: int, product_id: int, rating: int, comment: str = None
    ):
        session = self.Session()
        try:
            existing = (
                session.query(self.Rating)
                .filter_by(login_id=login_id, product_id=product_id)
                .first()
            )
            if existing:
    ...
    def get_rating(self, product_id: int):
        ...
        try:
            ratings = (
                session.query(self.Rating, self.Login.UserName)
                .join(self.Login, self.Rating.login_id == self.Login.loginid)
                .filter(self.Rating.product_id == product_id)
                .all()
            )
            if not ratings:
                return {"message": "No ratings yet."}

            avg = sum([rating.rating for rating, username in ratings]) / len(ratings)
