...
class category_repo(db_class):
    ...
    def getdata(self,category_id):
        ...
        try:
            category_products={}
            item = session.query(self.category).filter_by(category_id=category_id).first()
            if item:
                category_info = {
                ...
                category_products["category_products"]= item_repo().view_all_items_by_category(category_id)
    ...
    def get_category_name_by_id(self, category_id):
        session = self.Session()
        try:
            category = session.query(self.category).filter_by(category_id=category_id).first()
            if category:
