...
class buy_history_repo(db_class):
    ...
    class BuyHistory(Base):
        ...
        delivared_address = Column(Integer, ForeignKey('user_address.address_id', ondelete="CASCADE"), nullable=False)
    ...
    def delete_buy_item(self, login_id):
        session=self.Session()
        try:
            bought_item = session.query(self.BuyHistory).filter_by(login_id=login_id).first()
            if bought_item:
    ...
    def get_buy_history(self, login_id):
        session=self.Session()
        try:
            history_all=session.query(self.BuyHistory).filter_by(login_id=login_id).all()
            if history_all:
                history_list=[]
