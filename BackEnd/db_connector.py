...
def view_all_items():
    ...
    for item in items:
        print(
            f"ID: {item.product_id} | Name: {item.product_name} | MRP: ₹{item.MRP} | Discount: {item.discount}% | Stock: {item.stock_avl} | Free Delivery: {'Yes' if item.free_delivery_status else 'No'} | Category ID: {item.item_category} | Image: {item.product_image} | Policies: {item.applicable_policies} | Specs: {item.specs}"
        )
    print("\n End of List\n")
