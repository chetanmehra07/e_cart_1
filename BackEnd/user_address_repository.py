...
class user_address_repo(db_class):
    ...
    def delete_address(self, address_id):
        session=self.Session()
        try:
            address = session.query(self.UserAddress).filter_by(address_id=address_id).first()
            if address:
    ...
    def get_addresses_by_login(self, login_id):
        session=self.Session()
        try:
            address_all=session.query(self.UserAddress).filter_by(login_id=login_id).all()
            address_list=[]
