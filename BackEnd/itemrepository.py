...
class item_repo(db_class):
    ...
    def view_all_items_by_category(self,category_id):
        session = self.Session()
        try:
            items = session.query(self.ListItem).filter_by(item_category=category_id).all()
            items_list = []
            for item in items:
                item_dict = {
                    "product_id": item.product_id,
    ...
    def view_all_details_of_item(self,product_id):
        ...
        try:
            ...
            if item:
                ...
                policy_details = policy_repo().get_policy_details_from_productid(item.applicable_policies)
